    ...
    def on_tick(self, record: record.Record):
        if record is not None:
            if record.counter > 0 and (record.counter  % self.frequency == 0 or self.requote):
                ...
                self.quoter.q = self.position.total_qty * 5.0 / self.position.initial_balance
                self.quoter.tau = ((self.curr_step * self.frequency * self.data_frequency) % (self.total_time * 1000)) / (self.total_time * 1000)
